  ...
  try {
    ...
    globalThis.fetch = async (url, options) => {
      ...
      if (geminiCall === 1) {
        ...
        assert(systemPrompt.includes("どの診断でも同じ「モビー」として、やさしく親しみやすく返す"), "Gemini prompt should keep one Mobby persona");
        assert(systemPrompt.includes("親しみやすい雰囲気にする"), "Gemini prompt should include friendly tone rule");
        assert(systemPrompt.includes("稀に絵文字を自然に1個使う"), "Gemini prompt should keep emoji use rare");
...
process.env.LINE_AI_REPLY_DELAY_MS = "0";
