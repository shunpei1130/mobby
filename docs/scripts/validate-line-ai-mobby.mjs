...
async function callLiffPageStaticCheck() {
  ...
  assert(page.includes("showTikTokExternalBrowserGuide"), "LIFF link page should guide TikTok users to an external browser instead of opening LINE directly");
...
async function callSharedCtaStaticCheck() {
  ...
  assert(cta.includes("renderTikTokExternalBrowserGuide"), "diagnosis CTA should guide TikTok users to an external browser");
  assert(cta.includes("if (isTikTokInAppBrowser()) return;"), "diagnosis CTA should not prefetch LIFF sessions inside TikTok");
...
async function callKnowledgeReplyFlow() {
  ...
  const cases = [
    ...
    {
      message: "診断結果",
      history: [{ role: "assistant", text: FIRST_MESSAGE_TEXT }],
      user: linkedUser,
      reply: "もちろん。あなたは恋愛モビー診断の夜風のロマンチストだよ。自由もときめきも大事にする感じ、かなり出てる🙂",
      check(systemPrompt) {
        assert(systemPrompt.includes("個別診断結果の質問文脈"), "bare result prompt should include own-result conversation context");
        assert(systemPrompt.includes("短い聞き方"), "bare result prompt should treat short result text as an own-result question");
        assert(systemPrompt.includes("夜風のロマンチスト"), "bare result prompt should include saved result name");
      }
    },
    {
      message: "診断結果",
      ...
      reply: "まだ診断結果は連携されてないみたい。結果ページからLINE連携すると見られるよ。",
      check(systemPrompt) {
        assert(systemPrompt.includes("個別診断結果の質問文脈"), "bare unlinked result prompt should include own-result conversation context");
        assert(systemPrompt.includes("診断結果が未連携"), "bare unlinked result prompt should include unlinked result guidance");
        assert(systemPrompt.includes("診断結果ページからLINE連携"), "bare unlinked result prompt should guide result linking");
      }
    },
    {
      message: "連携済みだよ",
      history: [
        { role: "user", text: "私の診断結果は？" },
        { role: "assistant", text: "LINEでMobbyに連携してもらうと、あなたの診断結果を覚えておけるんだ。" }
      ],
      user: linkedUser,
      reply: "連携できてるよ。あなたの恋愛のタイプは「夜風のロマンチスト」だよ。",
      check(systemPrompt) {
        assert(systemPrompt.includes("個別診断結果の質問文脈"), "linked affirmation prompt should include own-result conversation context");
        assert(systemPrompt.includes("「連携済み」だけの返事"), "linked affirmation prompt should treat linked-only replies as result follow-ups");
        assert(systemPrompt.includes("夜風のロマンチスト"), "linked affirmation prompt should include saved result name");
      }
    },
    {
      message: "診断結果教えて",
...
async function callMockOwnResultFallbackFlow() {
  const originalProvider = process.env.AI_PROVIDER;
  const linkedUser = {
    source: "16renai",
    sourceLabel: "恋愛モビー診断",
  ...
  try {
    ...
    const bareReply = await generateReply({
      user: linkedUser,
      message: "診断結果",
      history: [{ role: "assistant", text: FIRST_MESSAGE_TEXT }]
    });
    assert(bareReply.includes("夜風のロマンチスト"), "mock fallback should answer bare result requests with saved result");
    assert(bareReply.includes("一緒に話そ"), "mock fallback should keep the conversation open");

    const linkedAffirmationReply = await generateReply({
      user: linkedUser,
      message: "連携済みだよ",
      history: [
        { role: "user", text: "私の診断結果は？" },
    ...
    assert(linkedAffirmationReply.includes("夜風のロマンチスト"), "mock fallback should answer linked-only follow-ups with saved result");
    ...
    assert(unlinkedReply.includes("診断結果ページからLINE連携"), "mock fallback should guide unlinked bare result requests to LINE linking");
  } finally {
...
await callMockOwnResultFallbackFlow();
