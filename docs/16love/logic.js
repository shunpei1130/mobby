...
function renderIntro() {
    ...
    app.innerHTML = `<div class="panel fade-in" style="text-align:center;padding:60px 20px;"><p class="kicker">メンヘラ度診断</p><h2 class="big">あなたのメンヘラタイプを<br>診断しよう💜</h2><p class="text-body" style="max-width:400px;margin:0 auto 30px;">直感で選ぶ40問の質問から、あなたのメンヘラ度とタイプを16種類で診断。メンヘラ度・依存度・自己演出度・回復力の4軸で徹底分析！</p><button class="primary" id="btnStart" style="padding:16px 32px;font-size:16px;">${c > 0 ? "診断を再開する" : "診断をはじめる"}</button>${c > 0 ? `<p style="font-size:12px;color:var(--text-sub);margin-top:16px;">${c}/40問 回答済み</p>` : ""}</div>`;
...
function renderResult() {
    ...
    const shareText = encodeURIComponent(`メンヘラモビー診断の結果は「${ch.name}」でした！💜\nメンヘラ度：Lv${res.level}（${res.menheraLevel.name}）\n${ch.catch}\n\nあなたも診断してみて👇`);
...
function renderCharacters() {
    ...
    app.innerHTML = `<div class="panel fade-in"><p class="kicker">全16タイプ</p><h2 class="big">キャラクター一覧</h2><p class="text-body" style="margin-bottom:20px;">4つの軸の組み合わせで生まれる、16種類のメンヘラタイプ。</p></div><div class="char-grid">${html}</div>`;
