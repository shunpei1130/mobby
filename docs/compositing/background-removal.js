(() => {
  ...
  // compositing/background-removal.src.js
  ...
  function ensureImageBitmapSource(input) {
    if (input instanceof HTMLImageElement || input instanceof HTMLCanvasElement || input instanceof HTMLVideoElement || input instanceof ImageBitmap || typeof OffscreenCanvas !== "undefined" && input instanceof OffscreenCanvas) {
      return input;
    }
  ...
  async function getImageSegmenter() {
    ...
    segmenterPromise = (async () => {
      const fileset = await Zo.forVisionTasks(WASM_ROOT);
      return Wc.createFromOptions(fileset, {
        baseOptions: {
  ...
  function findCropBounds(imageData, width, height) {
    ...
    for (let y2 = 0; y2 < height; y2 += 1) {
      for (let x2 = 0; x2 < width; x2 += 1) {
        const alpha = imageData.data[(y2 * width + x2) * 4 + 3];
  ...
  async function segmentPerson(input) {
    ...
    for (let i2 = 0; i2 < width * height; i2 += 1) {
      const offset = i2 * 4;
      const alpha = categoryValues[i2] === personCategoryIndex ? 255 : 0;
